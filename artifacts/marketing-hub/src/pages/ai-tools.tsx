...
import { Sparkles, Search, Share2, Globe, Megaphone, FileText, AlertCircle, Settings, HelpCircle, Code2, Copy, Check, Download, PenLine, Save, Zap, X, Network, MessageSquare, Send, Bot, User } from "lucide-react";
...
export default function AiTools() {
  ...
  const handleSendChat = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setChatMessages(prev => [...prev, { role: "assistant", content: `Error: ${err.error ?? "Something went wrong"}` }]);
      } else {
  ...
  function renderChatContent(content: string) {
    return content.split("\n").map((line, i) => {
      ...
      if (line.startsWith("# ")) return <p key={i} className="font-bold text-base mt-2 mb-0.5">{line.slice(2)}</p>;
      if (line.startsWith("- ") || line.startsWith("• ")) return <p key={i} className="text-sm pl-3 before:content-['•'] before:mr-2 before:-ml-3">{line.slice(2)}</p>;
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex gap-1 bg-muted rounded-lg p-1 shrink-0">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
              aiMode === "tools" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
              aiMode === "chat" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
            }`}
        ...
        <Card className="flex flex-col" style={{ height: "calc(100vh - 220px)", minHeight: 400 }}>
          <CardHeader className="pb-3 border-b shrink-0">
            <CardTitle className="text-base flex items-center gap-2">
              <Bot className="h-4 w-4 text-primary" /> SEO Assistant
            </CardTitle>
            <CardDescription className="text-xs">Ask me anything about SEO, content strategy, campaigns, or leads.</CardDescription>
          </CardHeader>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {chatMessages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-xs mt-1 opacity-70 max-w-sm">Ask about keyword strategy, content ideas, campaign planning, or any SEO & marketing question.</p>
                  <div className="flex flex-wrap gap-2 mt-4 justify-center">
                    {[
                      "How do I improve my keyword rankings?",
                      "Write a meta description for my homepage",
                      "What's a good content strategy for a SaaS?",
                      "Explain Google's E-E-A-T guidelines",
                    ].map(q => (
                      <button
                        key={q}
                        type="button"
              ...
              {chatMessages.map((msg, i) => (
                <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                  <div className={`shrink-0 h-7 w-7 rounded-full flex items-center justify-center text-white ${msg.role === "user" ? "bg-primary" : "bg-muted border"}`}>
                    {msg.role === "user"
                      ? <User className="h-3.5 w-3.5" />
                      : <Bot className="h-3.5 w-3.5 text-foreground" />
                    }
                  </div>
                  <div className={`max-w-[80%] rounded-xl px-3.5 py-2.5 ${msg.role === "user" ? "bg-primary text-primary-foreground ml-auto" : "bg-muted border"}`}>
                    {msg.role === "assistant" ? renderChatContent(msg.content) : <p className="text-sm">{msg.content}</p>}
                  </div>
                </div>
              ))}
              {chatLoading && (
                <div className="flex gap-3">
                  <div className="shrink-0 h-7 w-7 rounded-full bg-muted border flex items-center justify-center">
                    <Bot className="h-3.5 w-3.5 text-foreground" />
                  </div>
                  <div className="bg-muted border rounded-xl px-3.5 py-3 flex items-center gap-1.5">
            ...
            <div className="border-t p-3 shrink-0">
              <div className="flex gap-2">
                <Input
                  ...
                  placeholder={aiDisabled ? "AI is disabled — enable in Settings" : "Ask about SEO, content, campaigns…"}
                  value={chatInput}
                  onChange={e => setChatInput(e.target.value)}
                  onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSendChat(); } }}
                  disabled={aiDisabled || chatLoading}
                  className="flex-1"
                />
                <Button
                  size="icon"
              ...
              {chatMessages.length > 0 && (
                <button
                  type="button"
                  className="text-xs text-muted-foreground hover:text-foreground mt-1.5 ml-1 transition-colors"
      ...
      {aiMode === "tools" && <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 ${aiDisabled ? "opacity-60 pointer-events-none" : ""}`}>
        {/* 1. Keyword Suggester */}
